...
# Proxy sozlamalarini to'g'rilash
...
ADMIN_USERNAME = "Sening_Username" # 👈 SHU YERGA TELEGRAM USERNAME'INGIZNI YOZING (@ siz)
...
# 🔘 BOT BUYRUQLAR MENYUSI (Menyu tugmasida ko'rinib turadi)
try:
    bot.set_my_commands([
...
def bosh_menyu(user_id):
    ...
    btn4 = types.KeyboardButton(text="💬 Admin bilan bog'lanish")
...
# --- ADMIN BILAN BOG'LANISH HANDLERI ---
@bot.message_handler(func=lambda msg: msg.text == "💬 Admin bilan bog'lanish")
def contact_admin_handler(message):
    inline_kb = types.InlineKeyboardMarkup()
    btn_link = types.InlineKeyboardButton(text="👉 Adminga yozish (Lichka)", url=f"https://t.me/{ADMIN_USERNAME}")
...
def add_admin_start(message):
    if message.from_user.id in get_admins():
        msg = bot.send_message(message.chat.id, "👤 Yangi adminning <b>Telegram ID</b> raqamini kiriting:\n\n<i>ID raqamni olish uchun foydalanuvchi @myidbot ga yozishi kerak.</i>", reply_markup=bekor_qilish_menyusi())
...
# --- KANALLAR VA GURUHLARNI BOSHQARISH ---
